...
DEFAULT_CONFIG = REPO_ROOT / "pipelines" / "load" / "config" / "load.yml"
...
def load_config(path: str | os.PathLike | None = None) -> LoadConfig:
    ...
    return LoadConfig(
        destination=dest,
        source_root=_path(paths.get("source_root", "~/dev/data/extract/raw")),
        queue_dir=_path(paths.get("queue_dir", "~/dev/data/extract/_load_queue")),
