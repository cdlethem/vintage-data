...
def http_error(code, reason="failed", *, url=None):
    body = io.BytesIO(b'{"error":"upstream failure"}')
    error = urllib.error.HTTPError(
        url or selection_url(),
        code,
        reason,
...
class FetchWhoGhoODataTests(unittest.TestCase):
    ...
    def test_retries_retryable_http_errors_after_closing_response(self):
        error, error_body = http_error(503, "unavailable")
        response = JsonResponse(envelope([observation()], count=1))
        attempts = 0
    ...
    def test_non_retryable_and_exhausted_http_errors_close_and_propagate(self):
        for code, retries, expected_calls in ((404, 2, 1), (503, 1, 2)):
            with self.subTest(code=code):
                failures = [http_error(code) for _ in range(expected_calls)]
                errors = [failure[0] for failure in failures]
                with mock.patch.object(
                    MODULE.urllib.request, "urlopen", side_effect=errors
                ) as urlopen, mock.patch.object(MODULE.time, "sleep") as sleep:
                ...
                self.assertTrue(all(body.closed for _, body in failures))

    def test_main_http_failure_closes_response_without_stdout_or_url(self):
        authenticated_url = "https://user:secret@example.invalid/private"
        error, error_body = http_error(503, "upstream unavailable", url=authenticated_url)
        stdout = io.StringIO()
        stderr = io.StringIO()
        argv = [
            "--indicator", INDICATOR,
        ...
        self.assertIn("HTTP Error 503: upstream unavailable", stderr.getvalue())
        self.assertNotIn(authenticated_url, stderr.getvalue())
        self.assertNotIn("user:secret", stderr.getvalue())
