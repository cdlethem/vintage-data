...
class FetchMedicaidOpenDataTests(unittest.TestCase):
    ...
    def test_stdout_failure_leaves_only_an_uncommitted_pending_checkpoint(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(json.loads(payload)["id"], IDS[0])
            self.assertEqual(len(payload.encode("ascii")), state["pending"]["bytes"])
            self.assertFalse(MODULE.artifact_landed(state["pending"]))

    def test_digest_consistent_semantic_corruption_is_rejected_before_replay(self):
        old = "2026-09-17T12:00:00+00:00"
        revised = "2026-09-18T01:02:03Z"
        fetched_at = "2026-09-18T02:00:00+00:00"
        record_a = MODULE.normalize_record(dataset(0), fetched_at)
        record_b = MODULE.normalize_record(dataset(1), fetched_at)
        changed_a = MODULE.normalize_record(dataset(0, modified=revised), fetched_at)
        changed_b = MODULE.normalize_record(dataset(1, modified=revised), fetched_at)

        cases = {
            "first-run missing candidate": (
                {},
                {IDS[0]: old, IDS[1]: old},
                [record_a],
            ),
            "existing-state missing changed candidate": (
                {IDS[0]: old, IDS[1]: old},
                {IDS[0]: revised, IDS[1]: revised},
                [changed_a],
            ),
            "duplicate ID": ({}, {IDS[0]: old}, [record_a, record_a]),
            "unexpected ID": ({}, {IDS[0]: old}, [record_a, record_b]),
            "unchanged-only ID": (
                {IDS[0]: old, IDS[1]: old},
                {IDS[0]: old, IDS[1]: revised},
                [record_a, changed_b],
            ),
            "mismatched modified": ({}, {IDS[0]: revised}, [record_a]),
        }

        with tempfile.TemporaryDirectory() as directory, mock.patch.dict(
            os.environ, {"EXTRACT_DATA_ROOT": directory}
        ):
            for index, (name, (committed, candidates, records)) in enumerate(cases.items()):
                with self.subTest(name=name):
                    state_file = Path(directory) / f"corrupt-{index}.json"
                    ...
                    stdout = io.StringIO()
                    stderr = io.StringIO()
                    with mock.patch.object(MODULE, "fetch_catalog") as fetch, contextlib.redirect_stdout(
                        stdout
                    ...
                    self.assertEqual(stdout.getvalue(), "")
                    self.assertIn("semantic mismatch", stderr.getvalue())
                    self.assertEqual(state_file.read_bytes(), original)

    def test_matching_landed_artifact_cannot_promote_semantic_corruption(self):
        old = "2026-09-17T12:00:00+00:00"
        revised = "2026-09-18T01:02:03Z"
        fetched_at = "2026-09-18T02:00:00+00:00"
        record_a = MODULE.normalize_record(dataset(0), fetched_at)
        ...
        changed_b = MODULE.normalize_record(dataset(1, modified=revised), fetched_at)
        cases = {
            "first-run missing candidate": ({}, {IDS[0]: old, IDS[1]: old}, [record_a]),
            "existing-state missing changed candidate": (
                {IDS[0]: old, IDS[1]: old},
                {IDS[0]: revised, IDS[1]: revised},
                [changed_a],
        ...
        with tempfile.TemporaryDirectory() as directory, mock.patch.dict(
            ...
            raw = Path(directory) / "raw" / "source=medicaid_open_data" / "dt=2026-09-18"
            raw.mkdir(parents=True)
            for index, (name, (committed, candidates, records)) in enumerate(cases.items()):
                with self.subTest(name=name):
                    lines, digest, byte_count = MODULE.serialize_records(records)
                    payload = "\n".join(lines) + "\n"
                    state_file = Path(directory) / f"landed-corrupt-{index}.json"
                    corrupt = {
                        "version": 1,
                        "source": "medicaid_open_data",
                        "watermarks": committed,
                    ...
                    MODULE.save_state(state_file, corrupt)
                    original = state_file.read_bytes()
                    (raw / f"matching-{index}.ndjson").write_text(payload, encoding="ascii")

                    with self.assertRaisesRegex(ValueError, "semantic mismatch"):
                        MODULE.reconcile_landed_state(state_file, corrupt)
                    self.assertEqual(state_file.read_bytes(), original)

    def test_legacy_matching_artifact_requires_complete_candidate_changes(self):
        fetched_at = "2026-09-18T02:00:00+00:00"
        record_a = MODULE.normalize_record(dataset(0), fetched_at)
        lines, digest, byte_count = MODULE.serialize_records([record_a])
        payload = "\n".join(lines) + "\n"
        with tempfile.TemporaryDirectory() as directory, mock.patch.dict(
            os.environ, {"EXTRACT_DATA_ROOT": directory}
        ):
            ...
            legacy = {
                ...
                "pending": {
                    ...
                    "records": 1,
                    "watermarks": {
                        IDS[0]: record_a["modified"],
                        IDS[1]: dataset(1)["modified"],
                    },
                },
            }
            MODULE.save_state(state_file, legacy)
            original = state_file.read_bytes()
            raw = Path(directory) / "raw" / "source=medicaid_open_data" / "dt=2026-09-18"
            raw.mkdir(parents=True)
            (raw / "matching.ndjson").write_text(payload, encoding="ascii")
            stdout = io.StringIO()
            stderr = io.StringIO()
            with mock.patch.object(MODULE, "fetch_catalog") as fetch, contextlib.redirect_stdout(
                stdout
            ...
            fetch.assert_not_called()
            self.assertEqual(stdout.getvalue(), "")
            self.assertIn("missing changed IDs", stderr.getvalue())
            self.assertEqual(state_file.read_bytes(), original)

    def test_valid_mixed_unchanged_new_and_changed_candidates_replay_and_promote(self):
        old = "2026-09-17T12:00:00+00:00"
        revised = "2026-09-18T01:02:03Z"
        fetched_at = "2026-09-18T02:00:00+00:00"
        changed = MODULE.normalize_record(dataset(1, modified=revised), fetched_at)
        new = MODULE.normalize_record(dataset(2), fetched_at)
        records = [changed, new]
        lines, digest, byte_count = MODULE.serialize_records(records)
        payload = "\n".join(lines) + "\n"
        candidates = {
        ...
        with tempfile.TemporaryDirectory() as directory, mock.patch.dict(
            ...
            state = {
                "version": 1,
                "source": "medicaid_open_data",
                "watermarks": {IDS[0]: old, IDS[1]: old},
                "pending": {
                    "artifact_sha256": digest,
                    "bytes": byte_count,
                    "records": 2,
            ...
            self.assertEqual(MODULE.load_state(state_file), state)

            raw = Path(directory) / "raw" / "source=medicaid_open_data" / "dt=2026-09-18"
            raw.mkdir(parents=True)
            (raw / "matching.ndjson").write_text(payload, encoding="ascii")
            promoted = MODULE.reconcile_landed_state(state_file, MODULE.load_state(state_file))
            self.assertEqual(promoted["watermarks"], candidates)
    ...
    def test_real_runner_orders_checkpoint_after_raw_publication_evidence(self):
        with tempfile.TemporaryDirectory() as directory, CatalogServer(
            ...
            original_payload = failed_state["pending"]["payload"].encode("ascii")
            self.assertEqual(len(original_payload), failed_state["pending"]["bytes"])
            self.assertEqual(list(root.glob("raw/source=medicaid_open_data/dt=*/*.ndjson")), [])

            requests_before_retry = server.requests
            retry = RUNNER.run(config)
            self.assertEqual(retry["records"], 2)
            self.assertEqual(server.requests, requests_before_retry)
            artifacts = list(root.glob("raw/source=medicaid_open_data/dt=*/*.ndjson"))
            self.assertEqual(len(artifacts), 1)
            self.assertEqual(artifacts[0].read_bytes(), original_payload)
            self.assertEqual(json.loads(state_file.read_text(encoding="utf-8")), failed_state)
    ...
    def test_sink_failure_then_upstream_change_replays_before_new_snapshot(self):
        revised = dataset(0, modified="2026-09-18T01:02:03Z", title="Revised")
        with tempfile.TemporaryDirectory() as directory, CatalogServer(
            page(1, [dataset(0)])
        ) as server, mock.patch.dict(
            os.environ,
            {"EXTRACT_DATA_ROOT": directory, "MEDICAID_OPEN_DATA_URL": server.url},
        ):
            root = Path(directory)
            state_file = root / "state.json"
            config = self.runner_config(state_file)
            with mock.patch.object(RUNNER, "get_sink", return_value=FailingCommitSink(directory)):
                with self.assertRaisesRegex(OSError, "sink commit failure"):
                    RUNNER.run(config)
            pending = json.loads(state_file.read_text(encoding="utf-8"))
            original_payload = pending["pending"]["payload"].encode("ascii")

            server.document = page(1, [revised])
            requests_before_retry = server.requests
            self.assertEqual(RUNNER.run(config)["records"], 1)
            self.assertEqual(server.requests, requests_before_retry)
            artifact = next(root.glob("raw/source=medicaid_open_data/dt=*/*.ndjson"))
            self.assertEqual(artifact.read_bytes(), original_payload)
            self.assertEqual(json.loads(state_file.read_text(encoding="utf-8")), pending)

            self.assertEqual(RUNNER.run(config)["records"], 1)
            changed_pending = json.loads(state_file.read_text(encoding="utf-8"))
            self.assertEqual(
                json.loads(changed_pending["pending"]["payload"])["title"], "Revised"
            )
            self.assertEqual(RUNNER.run(config)["records"], 0)
            committed = json.loads(state_file.read_text(encoding="utf-8"))
            self.assertEqual(committed["watermarks"][IDS[0]], revised["modified"])
            self.assertNotIn("pending", committed)

    def test_sink_failure_then_upstream_removal_replays_before_empty_output(self):
        with tempfile.TemporaryDirectory() as directory, CatalogServer(
            page(1, [dataset(0)])
        ) as server, mock.patch.dict(
            os.environ,
            ...
            self.assertEqual(RUNNER.run(config)["records"], 0)
            committed = json.loads(state_file.read_text(encoding="utf-8"))
            self.assertEqual(committed["watermarks"], {})
            self.assertNotIn("pending", committed)
            self.assertEqual(RUNNER.run(config)["records"], 0)

    def test_sink_failure_then_upstream_reversion_preserves_each_candidate(self):
        revised = dataset(0, modified="2026-09-18T01:02:03Z", title="Revised")
        with tempfile.TemporaryDirectory() as directory, CatalogServer(
            page(1, [dataset(0)])
        ) as server, mock.patch.dict(
            ...
            with mock.patch.object(RUNNER, "get_sink", return_value=FailingCommitSink(directory)):
                with self.assertRaisesRegex(OSError, "sink commit failure"):
                    RUNNER.run(config)
            first_pending = json.loads(state_file.read_text(encoding="utf-8"))
            first_payload = first_pending["pending"]["payload"].encode("ascii")

            server.document = page(1, [revised])
            self.assertEqual(RUNNER.run(config)["records"], 1)
            first_artifact = next(root.glob("raw/source=medicaid_open_data/dt=*/*.ndjson"))
            self.assertEqual(first_artifact.read_bytes(), first_payload)
            with mock.patch.object(RUNNER, "get_sink", return_value=FailingCommitSink(directory)):
                with self.assertRaisesRegex(OSError, "sink commit failure"):
                    RUNNER.run(config)
            revised_pending = json.loads(state_file.read_text(encoding="utf-8"))
            revised_payload = revised_pending["pending"]["payload"].encode("ascii")
            self.assertEqual(json.loads(revised_payload)["modified"], revised["modified"])

            server.document = page(1, [dataset(0)])
            requests_before_retry = server.requests
            self.assertEqual(RUNNER.run(config)["records"], 1)
            self.assertEqual(server.requests, requests_before_retry)
            artifacts = list(root.glob("raw/source=medicaid_open_data/dt=*/*.ndjson"))
            self.assertIn(revised_payload, [artifact.read_bytes() for artifact in artifacts])
            self.assertEqual(json.loads(state_file.read_text(encoding="utf-8")), revised_pending)

            self.assertEqual(RUNNER.run(config)["records"], 1)
            reverted_pending = json.loads(state_file.read_text(encoding="utf-8"))
            self.assertEqual(
                json.loads(reverted_pending["pending"]["payload"])["modified"],
                dataset(0)["modified"],
            )
            self.assertEqual(RUNNER.run(config)["records"], 0)
            self.assertNotIn("pending", json.loads(state_file.read_text(encoding="utf-8")))
    ...
    def test_landed_checkpoint_promotion_failure_keeps_pending_payload_retryable(self):
        with tempfile.TemporaryDirectory() as directory, mock.patch.dict(
            os.environ, {"EXTRACT_DATA_ROOT": directory}
        ):
            state_file = Path(directory) / "state.json"
            ...
            raw = Path(directory) / "raw" / "source=medicaid_open_data" / "dt=2026-09-17"
            raw.mkdir(parents=True)
            (raw / "landed.ndjson").write_text(payload, encoding="ascii")
            pending = {
                "version": 1,
                "source": "medicaid_open_data",
                "watermarks": {},
            ...
            self.assertEqual(MODULE.load_state(state_file), pending)
            promoted = MODULE.reconcile_landed_state(state_file, MODULE.load_state(state_file))
            self.assertEqual(promoted["watermarks"], {IDS[0]: dataset(0)["modified"]})
            self.assertNotIn("pending", MODULE.load_state(state_file))

    def test_unlanded_legacy_or_corrupt_pending_payload_fails_explicitly(self):
        with tempfile.TemporaryDirectory() as directory, mock.patch.dict(
            os.environ, {"EXTRACT_DATA_ROOT": directory}
        ):
            state_file = Path(directory) / "state.json"
            record = MODULE.normalize_record(dataset(0), "2026-09-17T12:30:00+00:00")
            lines, digest, byte_count = MODULE.serialize_records([record])
            legacy = {
                "version": 1,
                "source": "medicaid_open_data",
                "watermarks": {},
            ...
            MODULE.save_state(state_file, legacy)
            stderr = io.StringIO()
            with mock.patch.object(MODULE, "fetch_catalog") as fetch, contextlib.redirect_stderr(stderr):
                with self.assertRaisesRegex(SystemExit, "2"):
                    MODULE.main(["--state-file", str(state_file)])
            fetch.assert_not_called()
            self.assertIn("cannot be reconstructed", stderr.getvalue())
            self.assertEqual(MODULE.load_state(state_file), legacy)

            corrupt = copy.deepcopy(legacy)
            corrupt["pending"]["payload"] = "{}\n"
            MODULE.save_state(state_file, corrupt)
            with self.assertRaisesRegex(ValueError, "does not match metadata"):
                MODULE.load_state(state_file)
