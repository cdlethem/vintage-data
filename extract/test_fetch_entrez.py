...
def article(pmid: str, *, title: str = "A climate study", include_journal: bool = True) -> bytes:
    journal = """
<Journal><JournalIssue><PubDate><Year>2026</Year><Month>Sep</Month></PubDate></JournalIssue><Title>Journal of Fixtures</Title></Journal>""" if include_journal else ""
...
def book_article(pmid: str, *, include_book: bool = True) -> bytes:
    book = """
<Book><BookTitle>Fixture Clinical Handbook</BookTitle><PubDate><Year>2025</Year><Month>Dec</Month><Day>9</Day></PubDate></Book>""" if include_book else ""
    return f'''<?xml version="1.0" encoding="UTF-8"?>
<PubmedArticleSet>
<PubmedBookArticle><BookDocument><PMID Version="1">{pmid}</PMID>{book}
<ArticleTitle>Climate chapter &amp; evidence</ArticleTitle>
<Abstract><AbstractText Label="SCOPE">Book abstract.</AbstractText></Abstract>
<AuthorList><Author><CollectiveName>Fixture Working Group</CollectiveName></Author></AuthorList>
<PublicationType>Book Chapter</PublicationType>
</BookDocument><PubmedBookData><ArticleIdList><ArticleId IdType="pubmed">{pmid}</ArticleId><ArticleId IdType="doi">10.5678/{pmid}</ArticleId></ArticleIdList></PubmedBookData></PubmedBookArticle>
</PubmedArticleSet>
'''.encode("utf-8")
...
class EntrezTests(unittest.TestCase):
    ...
    def test_discovery_emits_journal_and_book_records_with_variant_fields(self):
        journal_payload = article("201", title="Journal result")
        book_payload = book_article("202")
        records, _ = self.fetch(
            [search(2, ["201", "202"]), journal_payload, book_payload],
            retmax=2,
        )

        self.assertEqual(["201", "202"], [record["id"] for record in records])
        self.assertEqual({"2026-09-17T12:30:00+00:00"}, {record["fetched_at"] for record in records})
        self.assertEqual(
            [journal_payload, book_payload],
            [record["raw_payload"].encode("utf-8") for record in records],
        )
        book = records[1]
        self.assertEqual("Climate chapter & evidence", book["title"])
        self.assertEqual("SCOPE: Book abstract.", book["abstract"])
        self.assertEqual("Fixture Clinical Handbook", book["journal"])
        self.assertEqual("2025 Dec 9", book["publication_date"])
        self.assertEqual("10.5678/202", book["doi"])
        self.assertEqual("Fixture Working Group", book["authors"][0]["collective_name"])
        self.assertEqual(["Book Chapter"], book["publication_types"])

    def test_optional_journal_and_book_fields_may_be_absent(self):
        records, _ = self.fetch(
            [
                search(2, ["301", "302"]),
                article("301", include_journal=False),
                book_article("302", include_book=False),
            ],
            retmax=2,
        )
    ...
    def test_repeated_runs_preserve_both_payload_variants_and_stable_identifiers(self):
        payloads = [article("987654", title="Stable &amp; exact"), book_article("987655")]
        first, _ = self.fetch([search(2, ["987654", "987655"]), *payloads], retmax=2)
        second, _ = self.fetch(
            [search(2, ["987654", "987655"]), *payloads],
            retmax=2,
            now=lambda: datetime(2026, 9, 17, 13, 30, tzinfo=timezone.utc),
        )
        self.assertEqual(["987654", "987655"], [record["id"] for record in first])
        self.assertEqual([record["id"] for record in first], [record["id"] for record in second])
        self.assertEqual(
            [payload.decode("utf-8") for payload in payloads],
            [record["raw_payload"] for record in first],
        )
        self.assertEqual(
            [record["raw_payload"] for record in first],
            [record["raw_payload"] for record in second],
        )
    ...
    def test_malformed_search_fetch_and_api_errors_fail_without_main_output(self):
        ...
        bad_fetch = (
            ...
            (book_article("2"), "PMID mismatch"),
            (b"<PubmedArticleSet/>", "expected one PubMed record, received 0"),
            (
                b"<PubmedArticleSet><PubmedArticle/><PubmedBookArticle/></PubmedArticleSet>",
                "expected one PubMed record, received 2",
            ),
            (b"<PubmedArticleSet><UnknownRecord/></PubmedArticleSet>", "unsupported PubMed record variant"),
            (b"<PubmedArticleSet><PubmedArticle/></PubmedArticleSet>", "missing MedlineCitation"),
            (
                b"<PubmedArticleSet><PubmedArticle><MedlineCitation><PMID>1</PMID></MedlineCitation></PubmedArticle></PubmedArticleSet>",
                "missing Article",
            ),
            (b"<PubmedArticleSet><PubmedBookArticle/></PubmedArticleSet>", "missing BookDocument"),
        )
