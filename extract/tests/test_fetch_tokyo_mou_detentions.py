...
def table_document(*, valid=True, row=None):
...
class FetchDetentionsTests(unittest.TestCase):
    ...
    def test_short_row_fails_without_retries(self):
        row = [
            "1", "9876543", "Example", "Panama", "2001", "12345", "Cargo",
            "Class", "RO", "Company", "Tokyo", "2026-09-01", "",
        ]
        with mock.patch.object(MODULE.urllib.request, "urlopen", return_value=Response(table_document(row=row))) as urlopen, mock.patch.object(MODULE.time, "sleep") as sleep:
            with self.assertRaisesRegex(ValueError, "detention row has an unexpected shape"):
                list(MODULE.fetch_detentions(year=2026, month=9))

        urlopen.assert_called_once()
        sleep.assert_not_called()

    def test_overlong_row_fails_without_retries(self):
        row = [
            "1", "9876543", "Example", "Panama", "2001", "12345", "Cargo",
            "Class", "RO", "Company", "Tokyo", "2026-09-01", "", "Deficiency", "Extra",
        ]
        with mock.patch.object(MODULE.urllib.request, "urlopen", return_value=Response(table_document(row=row))) as urlopen, mock.patch.object(MODULE.time, "sleep") as sleep:
            with self.assertRaisesRegex(ValueError, "detention row has an unexpected shape"):
                list(MODULE.fetch_detentions(year=2026, month=9))
