...
def _query_landed(service: LoaderService):
...
def run_live() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="inaturalist-live-") as directory:
        ...
        try:
            ...
            rows = _query_landed(service)
            if len(rows) != len(records) or any(row[1] != SOURCE for row in rows):
                raise LiveFailure(70, "loader_readback_failure", "row count/source mismatch")
        finally:
            service.destination.close()
    print(
        json.dumps(
