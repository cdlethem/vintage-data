...
class PaleobiologyDatabaseTests(unittest.TestCase):
    ...
    def test_http_failure_has_status_and_bounded_redacted_diagnostic(self):
        secret = "TOP-SECRET-VALUE"
        body = (
            "upstream failed Authorization: Bearer "
            + secret
            + " URL=https://user:password@example.test/path?api_key="
            + secret
            + " "
            + ("x" * 5000)
        )
        with LocalFixture([(503, "text/plain", body)]) as fixture:
            child = run_cli(
                fixture.url,
                "--page-size",
                "2",
