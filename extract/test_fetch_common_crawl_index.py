...
CONFIG = Path(__file__).parent / "sources" / "common_crawl_index.yml"
...
def test_capture_id_distinguishes_each_stable_coordinate():
    common = cdx_row(
    ...
    variants = [
        (common, "CC-MAIN-2026-39"),
        ({**common, "urlkey": "com,example)/other"}, "CC-MAIN-2026-39"),
        ({**common, "timestamp": "20260922123456"}, "CC-MAIN-2026-39"),
        ({**common, "filename": "crawl-data/other.warc.gz"}, "CC-MAIN-2026-39"),
        ({**common, "offset": "9999"}, "CC-MAIN-2026-39"),
        (common, "CC-MAIN-2026-40"),
    ]
    records = [
        ...
        for row, crawl_id in variants
    ]

    assert len({record["id"] for record in records}) == len(variants)
...
def test_main_emits_compact_ndjson_using_injected_collection_endpoint(capsys):
    with LocalAPI() as api:
        def respond(path):
            ...
            if query(path).get("showNumPages") == ["true"]:
                return 200, {}, {"pages": 1}
            return 200, {}, ndjson(cdx_row(2), cdx_row(1))
