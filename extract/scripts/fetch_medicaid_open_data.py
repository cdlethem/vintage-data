...
Legacy digest-only pending checkpoints can be promoted when their matching raw artifact
already exists, but cannot reconstruct an unlanded historical payload and fail explicitly.
...
def load_state(path: pathlib.Path) -> dict[str, Any]:
    ...
    if pending is not None:
        ...
        if "payload" in pending:
            ...
            if (
                len(encoded) != pending["bytes"]
                or hashlib.sha256(encoded).hexdigest() != pending["artifact_sha256"]
            ...
            try:
                replay_records = [json.loads(line) for line in payload.splitlines()]
            except json.JSONDecodeError as error:
                raise ValueError(
                    f"malformed state in {path}: pending replay payload is not NDJSON"
                ) from error
            for record in replay_records:
                if (
                    ...
                    or pending["watermarks"].get(record.get("id")) != record.get("modified")
                ):
                    raise ValueError(
...
def replay_pending_payload(path: pathlib.Path, state: dict[str, Any]) -> str:
    """Return an exact unlanded payload, rejecting unreplayable legacy state."""
