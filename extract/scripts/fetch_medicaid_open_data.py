...
after) the local raw sink has published the exact emitted artifact. Before output, a
pending checkpoint atomically retains the exact NDJSON payload, its digest, byte count,
record count, and candidate watermarks while preserving committed watermarks. An
unlanded payload is replayed byte-for-byte before any newer catalog state is fetched.

Legacy digest-only pending checkpoints require their matching raw artifact bytes for
the same semantic validation; unlanded historical payloads cannot be reconstructed.
...
def _expected_emitted_ids(
    committed: dict[str, str], candidates: dict[str, str]
) -> set[str]:
    return {
        dataset_id
        for dataset_id, modified in candidates.items()
        if committed.get(dataset_id) != modified
    }
...
def _validate_pending_payload(
    ...
    """Validate exact bytes and the candidate changes those bytes represent."""
    if (
        len(payload_bytes) != pending["bytes"]
        or hashlib.sha256(payload_bytes).hexdigest() != pending["artifact_sha256"]
        or not payload_bytes.endswith(b"\n")
        or payload_bytes.count(b"\n") != pending["records"]
    ):
        raise ValueError(
            f"malformed state in {path}: pending replay payload does not match metadata"
        )
    try:
        ...
        raise ValueError(
            f"malformed state in {path}: pending replay payload is not ASCII NDJSON"
        ) from error

    payload_ids: list[str] = []
    mismatched_modified: set[str] = set()
    candidates = pending["watermarks"]
    for record in records:
        if (
            not isinstance(record, dict)
            or record.get("source") != SOURCE
            or not _valid_uuid(record.get("id"))
            or not isinstance(record.get("fetched_at"), str)
        ):
            raise ValueError(
                f"malformed state in {path}: pending replay payload has invalid envelope"
            )
        dataset_id = record["id"]
        payload_ids.append(dataset_id)
        if candidates.get(dataset_id) != record.get("modified"):
            mismatched_modified.add(dataset_id)

    expected = _expected_emitted_ids(committed, candidates)
    observed = set(payload_ids)
    has_duplicates = len(payload_ids) != len(observed)
    problems: list[str] = []
    if has_duplicates:
        problems.append("duplicate IDs")
    if expected - observed:
        problems.append("missing changed IDs")
    if observed - expected:
        problems.append("unexpected or unchanged IDs")
    if mismatched_modified:
        problems.append("modified values differ from candidate watermarks")
    if problems:
        raise ValueError(
            f"malformed state in {path}: pending replay payload semantic mismatch: "
            + "; ".join(problems)
        )
    return payload
...
def load_state(path: pathlib.Path) -> dict[str, Any]:
    ...
    if pending is not None:
        ...
        if "payload" in pending:
            try:
                payload_bytes = pending["payload"].encode("ascii")
            except (AttributeError, UnicodeEncodeError) as error:
                raise ValueError(
                    f"malformed state in {path}: invalid pending replay payload"
                ) from error
            _validate_pending_payload(path, pending, state["watermarks"], payload_bytes)
    return state


def save_state(path: pathlib.Path, state: dict[str, Any]) -> None:
    """Atomically and durably publish committed or pending state."""
    ...
    try:
        ...
        directories_to_sync.extend(directory.parent for directory in missing_directories)
...
def replay_pending_payload(path: pathlib.Path, state: dict[str, Any]) -> str:
    """Return an exact, semantically valid unlanded payload."""
    pending = state["pending"]
    if "payload" not in pending:
        raise ValueError(
            f"pending checkpoint in {path} predates replay-payload persistence; "
            "its unlanded historical payload cannot be reconstructed"
        )
    try:
        payload_bytes = pending["payload"].encode("ascii")
    except (AttributeError, UnicodeEncodeError) as error:
        raise ValueError(f"pending checkpoint in {path} has invalid replay bytes") from error
    return _validate_pending_payload(path, pending, state["watermarks"], payload_bytes)
...
def _matching_landed_payload(pending: dict[str, Any]) -> bytes | None:
    """Return exact matching local-sink bytes, if a final artifact exists."""
    ...
    try:
        ...
        for artifact in artifacts:
            try:
                ...
                if hashlib.sha256(payload).hexdigest() == pending["artifact_sha256"]:
                    return payload
...
def artifact_landed(pending: dict[str, Any]) -> bool:
    ...
    return _matching_landed_payload(pending) is not None


def reconcile_landed_state(path: pathlib.Path, state: dict[str, Any]) -> dict[str, Any]:
    """Promote a semantically valid pending snapshot after exact raw landing."""
    pending = state.get("pending")
    if pending is None:
        return state
    landed_payload = _matching_landed_payload(pending)
    if landed_payload is None:
        return state
    _validate_pending_payload(path, pending, state["watermarks"], landed_payload)
    committed = {
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    try:
        ...
        if path is not None:
            ...
            if "pending" in state:
                sys.stdout.write(replay_pending_payload(path, state))
                sys.stdout.flush()
                return
