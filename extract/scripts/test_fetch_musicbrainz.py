...
class FetchMusicBrainzTests(unittest.TestCase):
    ...
    def test_very_long_delta_seconds_retry_after_recovers_with_bounded_waits(self):
        cases = (
            ("9" * 5_000, 30.0),
            ("0" * 5_000 + "5", 5.0),
            ("0" * 5_000 + "999", 30.0),
        ...
        for retry_after, expected_delay in cases:
            with self.subTest(retry_after_length=len(retry_after), expected_delay=expected_delay):
                self.setUp()
                clock = Clock()
                starts = []
                responses = iter((
                ...
                with mock.patch.object(MODULE.time, "monotonic", clock.monotonic), mock.patch.object(
                    MODULE.time, "sleep", clock.sleep
                ), mock.patch.object(MODULE.urllib.request, "urlopen", side_effect=urlopen) as mocked_urlopen:
                    self.assertEqual(MODULE._get("https://example.test/query"), {"releases": []})

                self.assertEqual(mocked_urlopen.call_count, 3)
                self.assertEqual(starts, [0.0, expected_delay, expected_delay * 2])
                self.assertEqual(clock.sleeps, [expected_delay, expected_delay])
                self.assertLessEqual(sum(clock.sleeps), 60.0)
