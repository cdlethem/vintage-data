...
class FetchJobBoardsSummaryTests(unittest.TestCase):
    def test_high_cardinality_workday_outcomes_use_bounded_aggregate_summary(self):
        ...
        diagnostics = [line for line in lines if not line.startswith(MODULE.SUMMARY_PREFIX)]
        ...
        self.assertEqual(summary["records"], attempted - failed)
        ...
        self.assertEqual(summary["partitions"]["failure_sample_count"], MODULE.MAX_FAILURE_SAMPLES)
        self.assertEqual(len(summary["partitions"]["failure_sample"]), MODULE.MAX_FAILURE_SAMPLES)
        self.assertEqual(summary["partitions"]["failures_omitted"], failed - MODULE.MAX_FAILURE_SAMPLES)
