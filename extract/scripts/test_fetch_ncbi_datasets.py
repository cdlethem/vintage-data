...
MALFORMED_API_KEY = "sensitive-crlf-key\r\nX-Injected: evil"
...
def test_callable_rejects_header_unsafe_api_key_before_transport():
    ...
    assert "sensitive-crlf-key" not in message
...
def test_callable_rejects_header_unsafe_api_key_from_environment():
    ...
    opener.assert_not_called()
    assert MALFORMED_API_KEY not in str(raised.value)
...
def test_cli_rejects_header_unsafe_api_key_before_transport():
    ...
    assert "sensitive-crlf-key" not in stdout
    assert "sensitive-crlf-key" not in stderr
