...
class Response(io.BytesIO):
    ...
    def read(self, size=-1):
        if fixture.get("failure_kind") == "incomplete_read":
            detail = fixture.get("failure_detail", "fixture failure").encode("utf-8")
            raise http.client.IncompleteRead(detail, len(detail) + 1)
...
def open_fixture(request, **kwargs):
    ...
    if failure_kind == "bad_status":
        raise http.client.BadStatusLine(failure_detail)
...
def test_http_protocol_failures_discard_sensitive_exception_chains(failure_phase):
    sensitive_reason = "DISTINCTIVE_HTTP_PROTOCOL_REASON"
    sensitive_url = "DISTINCTIVE_HTTP_PROTOCOL_AUTHENTICATED_URL"
    detail = f"{sensitive_reason}:{sensitive_url}:{API_KEY}"
    if failure_phase == "open":
        opener = mock.Mock(side_effect=http.client.BadStatusLine(detail))
    else:
        class IncompleteResponse(JsonResponse):
            def read(self, size=-1):
                partial = detail.encode("utf-8")
                raise http.client.IncompleteRead(partial, len(partial) + 1)

        opener = mock.Mock(return_value=IncompleteResponse(document()))

    with pytest.raises(MODULE.NCBIDatasetsError) as raised:
        MODULE.fetch_genome_reports([ACCESSION], api_key=API_KEY, opener=opener)
    ...
    assert str(error) == f"request for {ACCESSION} failed"
    assert error.__cause__ is None
    assert error.__context__ is None
    assert error.__suppress_context__ is True
    for secret in (API_KEY, sensitive_reason, sensitive_url):
...
@pytest.mark.parametrize("failure_kind", ["bad_status", "incomplete_read"])
def test_cli_http_protocol_failures_do_not_disclose_credentials(failure_kind):
    api_key = "complete-synthetic-cli-http-protocol-api-key"
    sensitive_fragment = "DISTINCTIVE_CLI_HTTP_PROTOCOL_FRAGMENT"
    detail = f"{sensitive_fragment}--{api_key}"
    code, stdout, stderr = run_cli(
        [ACCESSION],
        failure_kind=failure_kind,
    ...
    assert code != 0
    assert stdout == ""
    assert f"ncbi_datasets: transport_error: request for {ACCESSION} failed" in stderr
    assert api_key not in stdout
    assert api_key not in stderr
    assert sensitive_fragment not in stdout
    assert sensitive_fragment not in stderr
