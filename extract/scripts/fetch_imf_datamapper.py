...
_CREDENTIAL_LABEL_RE = (
    r"authorization|proxy-authorization|api[-_]?key|access[-_]?token|"
    r"token|password|secret"
)
_SECRET_RE = re.compile(
    rf"(?i)\b({_CREDENTIAL_LABEL_RE})\s*([:=])\s*"
    rf"(?:bearer\s+|basic\s+)?[^\s&]+"
)
_CREDENTIAL_OPTION_RE = re.compile(rf"(?i)--(?:{_CREDENTIAL_LABEL_RE})")
_CREDENTIAL_INLINE_OPTION_RE = re.compile(
    rf"(?is)(--(?:{_CREDENTIAL_LABEL_RE}))([=:]).*"
)
...
def _redact_credential_arguments(argv: Sequence[str]) -> list[str]:
    """Redact credential values at argv boundaries before argparse joins them."""
    safe_argv = list(argv)
    redact_next = False
    for index, argument in enumerate(safe_argv):
        if redact_next:
            safe_argv[index] = "[redacted]"
            redact_next = False
            continue

        inline = _CREDENTIAL_INLINE_OPTION_RE.fullmatch(argument)
        if inline:
            safe_argv[index] = f"{inline.group(1)}{inline.group(2)}[redacted]"
        elif _CREDENTIAL_OPTION_RE.fullmatch(argument):
            redact_next = True
    return safe_argv

class SafeArgumentParser(argparse.ArgumentParser):
    """Argument parser whose failures use the extractor diagnostic boundary."""

    def error(self, message: str) -> None:
        detail = _safe_diagnostic_detail(message)
        self.exit(2, f"fetch_imf_datamapper: argument error: {detail}\n")
