...
def test_populated_feed_normalizes_units_times_links_and_preserves_unknown_fields():
    payload = document([storm(), storm(id="ep012026", movementDir="WNW")])
    records, urlopen = fetch(payload)

    assert len(records) == 2
    assert [item["movement_direction"] for item in records] == ["NW", "WNW"]
    record = records[0]
    assert record == {
        ...
        "raw_storm": payload["activeStorms"][0],
...
def test_explicit_null_movement_direction_remains_nullable():
    record = fetch(document([storm(movementDir=None)]))[0][0]

    assert record["movement_direction"] is None
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (document([storm(movementDir=315)]), "movementDir must be a string or null"),
    ],
)
...
def test_repository_raw_loading_and_actual_model_sql_preserve_snapshots_types_and_replay():
    with tempfile.TemporaryDirectory() as directory:
        ...
        second_storm = storm(movementDir="WNW")
        ...
        rows = connection.execute(
            ...
            select storm_id, fetched_at, advisory_at, intensity_kt, pressure_mb,
                   latitude, longitude, movement_direction, movement_speed_kt,
                   typeof(fetched_at), typeof(advisory_at), typeof(intensity_kt),
                   typeof(movement_direction),
    ...
    assert rows[0][3:9] == (85.0, 972.0, 24.5, -63.2, "NW", 9.0)
    assert rows[1][7] == "WNW"
    assert rows[0][9:13] == (
...
def test_model_documentation_names_units_payloads_and_distinct_times():
    ...
    assert columns["movement_direction"]["data_type"] == "varchar"
    assert "compass" in columns["movement_direction"]["description"]
    assert "non-text" in columns["movement_direction"]["description"]
