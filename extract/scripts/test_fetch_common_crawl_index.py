...
class BlockingResponse:
    def __init__(self, *, url, payload=b"", delay=None):
        self.headers = {}
        self.status = 200
        self._url = url
    ...
    def read(self, size=-1):
        ...
        elif self.released.wait(self._delay):
            return b""
        if self.released.is_set() or self._offset >= len(self._payload):
            return b""
        value = self._payload[self._offset:self._offset + 1]
        self._offset += 1
        return value
...
def test_stalled_read_is_interrupted_closed_and_reported_at_absolute_deadline():
    response = BlockingResponse(url=MODULE.DISCOVERY_URL)
    ...
    assert response.released.is_set()
    assert response.read_calls == 1
    assert len(opener.calls) == 1
    assert MODULE.LAST_RUN_METADATA["requests"]["failed"] == 1


def test_drip_fed_read_cannot_refresh_deadline_or_escape_bounded_work():
    response = BlockingResponse(
        url=MODULE.DISCOVERY_URL,
        payload=json.dumps(discovery()).encode("utf-8"),
        delay=0.02,
    )
    opener = QueueOpener([response])
    started = time.monotonic()
