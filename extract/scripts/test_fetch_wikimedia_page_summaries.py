...
class WikimediaWarehouseIntegrationTests(unittest.TestCase):
    ...
    @staticmethod
    def _run_child(argv, *, cwd, env, redact):
        ...
        if completed.returncode:
            output = (completed.stdout + "\n" + completed.stderr).replace(str(redact), "<tmp>")
            output = re.sub(
                r"(?i)(token|password|secret)=\S+", r"\1=<redacted>", output
            )
            raise AssertionError(
                f"child exited {completed.returncode}: {' '.join(argv[:4])}\n{output[-4000:]}"
            )
        return completed
