...
def fetch(kind: str = "food_recall", days_back: int = 14, limit: int = 100):
    ...
    data = _get(path, {
        # a literal space here, not "+" -- urlencode() would otherwise escape
        # the "+" itself (-> %2B), which the server reads as a literal
        # plus character rather than the space its query syntax expects.
