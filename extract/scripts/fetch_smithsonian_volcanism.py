...
def _fetch_document(params, requested_count, diagnostics=None):
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        ...
        if failure_kind is None:
            body = b"".join(chunks)
            attempt_bytes = len(body)
        else:
            attempt_bytes = sum(len(chunk) for chunk in chunks)
            body = b""
        ...
        if failure_kind == "transport" and failure_phase == "read" and elapsed >= REQUEST_BUDGET_SECONDS:
            _emit_attempt(
                attempt=attempt,
                elapsed=elapsed,
                status=status,
            ...
            fail("request deadline exhausted after response read failure", attempt, "read_deadline")
