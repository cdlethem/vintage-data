...
def _retry_after_delay(value, now):
    ...
    if isinstance(value, str) and value:
        ...
        for index, char in enumerate(value):
            if char < "0" or char > "9":
                break
            if char != "0" and first_significant is None:
                first_significant = index
        else:
            if first_significant is None:
                return _MIN_RETRY_DELAY
            max_delay = int(_MAX_RETRY_DELAY)
            significant_digits = len(value) - first_significant
            max_delay_digits = str(max_delay)
            if (significant_digits > len(max_delay_digits)
                    or (significant_digits == len(max_delay_digits)
                        and value[first_significant:] > max_delay_digits)):
                return _MAX_RETRY_DELAY
            return max(_MIN_RETRY_DELAY, int(value[first_significant:]))
    try:
...
def _get(url):
    ...
    seconds of retry waiting across task attempts. These retry-wait bounds exclude
    HTTP durations: urllib's 30-second timeout applies to blocking socket operations,
    not a total HTTP-call deadline. The query loop makes one request sequence per
    ``search_releases`` call, so callers that issue multiple queries have no aggregate
    retry-wait bound. The existing ten-minute Airflow task timeout can still interrupt
    a task, and no completion-duration guarantee is compatible with the socket timeout
    semantics; neither timeout behavior nor task configuration is changed here.
    """
    retry_wait = 0.0
