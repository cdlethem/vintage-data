...
API_BASE = os.environ.get("NCBI_DATASETS_API_BASE", OFFICIAL_API_BASE).rstrip("/")
...
def request_url(accession: str) -> str:
    """Return the configured request endpoint; production defaults to the official API."""
    encoded = urllib.parse.quote(accession, safe="")
    return f"{API_BASE}/{encoded}/dataset_report"
    ...
    *,
    ...
    for index, accession in enumerate(requested):
        ...
        try:
            ...
            raise NCBIDatasetsError(
                "http_error",
                f"request for {accession} returned HTTP {error.code}",
            ) from error
        except NCBIDatasetsError:
            raise
        except (urllib.error.URLError, TimeoutError, OSError) as error:
            raise NCBIDatasetsError(
                "transport_error", f"request for {accession} failed"
            ) from error
