...
def _parse_pubmed_payload(body: bytes, expected_pmid: str) -> dict[str, Any]:
    ...
    if len(articles) != 1:
        raise EntrezError(f"NCBI Entrez EFetch expected one PubmedArticle, received {len(articles)}")
    article = articles[0]
    pmid = _text(article.find("./MedlineCitation/PMID"))
    if pmid != expected_pmid:
        raise EntrezError(f"NCBI Entrez EFetch PMID mismatch: expected {expected_pmid}, received {pmid!r}")

    article_node = article.find("./MedlineCitation/Article")
    if article_node is None:
        raise EntrezError(f"NCBI Entrez EFetch PubmedArticle {pmid} is missing Article")
    abstract_parts = []
    ...
    publication_date = _text(journal_issue.find("./MedlineDate")) if journal_issue is not None else None
    if publication_date is None and journal_issue is not None:
        publication_date = " ".join(
            value
            for value in (
                _text(journal_issue.find("./Year")),
                _text(journal_issue.find("./Month")),
                _text(journal_issue.find("./Day")),
            )
            if value
        ) or None
    return {
