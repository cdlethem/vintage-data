...
class FetchIMFDataMapperTests(unittest.TestCase):
    ...
    def test_actual_cli_separate_credential_values_are_fully_redacted(self):
        ordinary_api_key = "ordinary-api-key-fixture"
        oversized_prefix = "oversized-api-key-prefix"
        cases = (
            ("--api-key", ordinary_api_key, ordinary_api_key),
            ("--api-key", oversized_prefix + ("z" * 5000), oversized_prefix),
            ("--authorization", "authorization fixture with spaces", "authorization fixture"),
            (
                "--proxy-authorization",
                "proxy-auth-fixture\nwith\ttabs\x1bcontrol",
                "proxy-auth-fixture",
            ),
            ("--apikey", "apikey-fixture", "apikey-fixture"),
            ("--api_key", "api-underscore-fixture", "api-underscore-fixture"),
            ("--access-token", "access-token-fixture", "access-token-fixture"),
            ("--accesstoken", "accesstoken-fixture", "accesstoken-fixture"),
            ("--access_token", "access-underscore-fixture", "access-underscore-fixture"),
            ("--token", "token-fixture", "token-fixture"),
            ("--password", "password-fixture", "password-fixture"),
            ("--secret", "secret-fixture", "secret-fixture"),
        )
        framing = len("fetch_imf_datamapper: argument error: \n")
        for option, value, sensitive_content in cases:
            with self.subTest(option=option, value_length=len(value)):
                completed = subprocess.run(
                    [sys.executable, str(SCRIPT), "NGDP_RPCH", option, value],
                    capture_output=True,
                    text=True,
                    timeout=10,
                    check=False,
                ...
                self.assertNotIn(sensitive_content, completed.stderr)
                self.assertNotIn("\t", completed.stderr)
                self.assertNotIn("\x1b", completed.stderr)
                self.assertEqual(len(completed.stderr.splitlines()), 1)
                self.assertNotIn("Traceback", completed.stderr)
