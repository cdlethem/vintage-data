...
class SummaryEmissionError(OSError):
    """The reserved run-summary line could not be fully emitted."""
...
def _request_document(url: str, timeout: float, metrics: dict[str, int]) -> Any:
    ...
    metrics["requests"] += 1
    metrics["attempted"] = metrics.get("attempted", 0) + 1
    try:
        ...
        raise
    metrics["succeeded"] = metrics.get("succeeded", 0) + 1
    metrics["response_bytes"] += len(raw)
    metrics["payload_bytes"] = metrics.get("payload_bytes", 0) + len(raw)
...
def _failure_message(exc: BaseException) -> str:
    ...
    message = re.sub(
        ...
        lambda match: urllib.parse.urlunsplit(
            (*urllib.parse.urlsplit(match.group(0))[:3], "<redacted>", "")
        ),
        message,
    )
...
def _mark_failed_partition(metrics: dict[str, Any], error: str) -> None:
    active = metrics.get("active_partition")
    if not isinstance(active, dict):
        return
    metrics["targets_failed"] = metrics.get("targets_failed", 0) + 1
    metrics.setdefault("partition_failures", []).append({**active, "error": error})
    metrics["active_partition"] = None
...
def run_summary(
    ...
    *,
    ...
    requests_attempted = request_metrics.get("attempted", request_metrics.get("requests", 0))
    requests_failed = request_metrics.get("failed", 0)
    requests_succeeded = request_metrics.get("succeeded", requests_attempted - requests_failed)
    targets_attempted = metrics.get("targets_attempted", 0)
    targets_succeeded = metrics.get("targets_completed", 0)
    targets_failed = metrics.get("targets_failed", 0)
    summary: dict[str, Any] = {
        ...
        "requests": {
            ...
            "payload_bytes": request_metrics.get("payload_bytes", request_metrics.get("response_bytes", 0)),
        },
        "partitions": {
            "attempted": targets_attempted,
            "succeeded": targets_succeeded,
            "failed": targets_failed,
            "failures": list(metrics.get("partition_failures", [])),
        },
        "coverage": {
            ...
            "targets_remaining": max(0, targets - targets_succeeded - targets_failed),
        ...
        "metrics": {"records_emitted": records},
    }
    if error is not None:
        summary["metrics"]["error"] = error
    return summary


def _emit_summary(err: Any, summary: dict[str, Any]) -> None:
    try:
        err.write(SUMMARY_PREFIX + json.dumps(summary, separators=(",", ":")) + "\n")
        err.flush()
    except Exception as exc:
        raise SummaryEmissionError(_failure_message(exc)) from exc
...
def main(argv: list[str] | None = None, *, out: Any = None, err: Any = None) -> int:
    ...
    try:
        ...
        _mark_failed_partition(metrics, error)
        summary = run_summary(
            metrics,
            records=emitted,
            targets=len(targets),
        ...
        try:
            _emit_summary(err, summary)
        except SummaryEmissionError:
            pass
        return 1
