...
class HttpClient:
    ...
    @staticmethod
    def _abort_response(response: Any) -> bool:
        """Abort a socket-backed response without acquiring its buffered read lock."""
        current = response
        transport = None
        for _ in range(4):
            raw = getattr(current, "raw", None)
            transport = getattr(raw, "_sock", None) or getattr(current, "_sock", None)
    ...
    def _run_blocking(
        ...
        *,
        ...
        """Run one blocking transport operation within the absolute run deadline."""
        ...
        def run() -> None:
            try:
                ...
                if cancelled.is_set() and hasattr(value, "close"):
                    value.close()
                outcome = (True, value)
            try:
        ...
        worker = threading.Thread(target=run, daemon=True, name="common-crawl-io")
        worker.start()
        ...
        if worker.is_alive():
            ...
            if worker.is_alive():
                raise CommonCrawlError(
                    f"{context} transport did not stop within the cleanup bound"
                )
    ...
    def _read_response(self, response: Any, context: str, allow_truncate: bool) -> tuple[bytes, str | None]:
        ...
        while len(body) < target:
            ...
            chunk = self._run_blocking(
                ...
                cancel=lambda: self._abort_response(response),
            )
            if not chunk:
                break
        ...
        for attempt in range(self.retries + 1):
            ...
            try:
                response = self._run_blocking(
                    lambda: self.opener(request, min(self.timeout, remaining)),
                    context=f"{context} connection",
                    operation_timeout=min(self.timeout, remaining),
                )
                with response:
            ...
            if delay:
                self._run_blocking(
                    lambda: self.sleeper(delay),
                    context=f"retry backoff for {context}",
