...
def _optional_integer(row: dict, key: str):
    ...
    try:
        ...
        raise CommonCrawlError(f"index record field {key!r} must be an integer") from exc
    if number < 0:
...
def normalize_record(
    ...
    *,
    ...
    return {
        "source": SOURCE,
        "id": f"{crawl_id}:{url_key}:{timestamp}",
        ...
        "warc_filename": _optional_string(row, "filename"),
