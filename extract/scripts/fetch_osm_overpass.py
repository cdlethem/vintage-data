...
def parse_bbox(value: str | Sequence[float]) -> tuple[Decimal, Decimal, Decimal, Decimal]:
    ...
    try:
        # Decimal retains the validated base-10 coordinates for the query. A
        # float conversion here could round or collapse close ordered bounds.
        coordinates = tuple(
    ...
    if not all(number.is_finite() for number in coordinates):
...
def build_query(
    ...
    bounds = ",".join(str(number) for number in (south, west, north, east))
