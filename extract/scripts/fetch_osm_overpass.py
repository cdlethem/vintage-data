...
def parse_bbox(value: str | Sequence[float]) -> tuple[float, float, float, float]:
    ...
    try:
        # Validate base-10 values before converting to query-builder floats.
        # Binary subtraction can make an exact 0.1-degree span appear too large;
        # validating rounded floats could instead admit a genuinely larger span.
        decimal_coordinates = tuple(
            Decimal(part) if isinstance(part, str) else Decimal(str(part))
            for part in parts
        )
    except (InvalidOperation, TypeError, ValueError) as error:
        raise ValueError("bbox coordinates must be finite numbers") from error
    if not all(number.is_finite() for number in decimal_coordinates):
        raise ValueError("bbox coordinates must be finite numbers")
    south, west, north, east = decimal_coordinates
    if not Decimal("-90") <= south <= Decimal("90") or not Decimal(
        "-90"
    ) <= north <= Decimal("90"):
        raise ValueError("bbox latitudes must be between -90 and 90")
    if not Decimal("-180") <= west <= Decimal("180") or not Decimal(
        "-180"
    ) <= east <= Decimal("180"):
    ...
    coordinates = tuple(float(number) for number in decimal_coordinates)
    south_float, west_float, north_float, east_float = coordinates
    if south_float >= north_float or west_float >= east_float:
        raise ValueError("bbox coordinates are too precise to represent")
    latitude_span = Fraction(north) - Fraction(south)
    longitude_span = Fraction(east) - Fraction(west)
    if (
        latitude_span > Fraction(str(MAX_LATITUDE_SPAN))
        or longitude_span > Fraction(str(MAX_LONGITUDE_SPAN))
        or latitude_span * longitude_span > Fraction(str(MAX_BBOX_AREA))
    ):
        raise ValueError(
