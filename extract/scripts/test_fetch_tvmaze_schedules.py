...
@pytest.mark.parametrize(
    ("urlopen_result", "urlopen_failure", "failure_category"),
    (
        (
            TruncatedResponse(b""),
            None,
...
def test_cli_handles_http_protocol_failures_without_partial_output_or_traceback(
    capsys, urlopen_result, urlopen_failure, failure_category
):
    with mock.patch.object(
        MODULE.urllib.request,
        "urlopen",
        return_value=urlopen_result,
        side_effect=urlopen_failure,
    ):
        status = MODULE.main(["--date", SCHEDULE_DATE])
    ...
    assert len(captured.err.splitlines()) == 1
    assert len(captured.err) <= MODULE.MAX_ERROR_DETAIL + len("error: \n")
    assert failure_category in captured.err
    assert "secret" not in captured.err
