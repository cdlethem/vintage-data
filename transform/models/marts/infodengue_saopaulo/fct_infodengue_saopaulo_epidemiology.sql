{{ config(
    ...
    unique_key='infodengue_saopaulo_observation_key',
...
with source_rows as (
    ...
    from {{ ref('base_infodengue_saopaulo') }}
    {% if is_incremental() %}
    where _loaded_at >= (
...
deduplicated as (
    select
        cast(md5(to_json(struct_pack(
            ...
            record_id := record_id
        ))) as varchar) as infodengue_saopaulo_observation_key,
        *,
        row_number() over (
...
select
    cast(infodengue_saopaulo_observation_key as varchar) as infodengue_saopaulo_observation_key,
