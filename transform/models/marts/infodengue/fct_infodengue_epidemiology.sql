{{ config(
    ...
    unique_key='infodengue_observation_key',
...
deduplicated as (
    select
        cast(md5(to_json(struct_pack(
            ...
            record_id := record_id
        ))) as varchar) as infodengue_observation_key,
        *,
        row_number() over (
...
select
    cast(infodengue_observation_key as varchar) as infodengue_observation_key,
