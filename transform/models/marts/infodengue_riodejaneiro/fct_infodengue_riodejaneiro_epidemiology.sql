...
with source_rows as (
    ...
    from {{ ref('base_infodengue_riodejaneiro') }}
),
...
deduplicated as (
    select
        cast(md5(to_json(struct_pack(
            ...
            record_id := record_id
        ))) as varchar) as infodengue_riodejaneiro_observation_key,
        *,
        row_number() over (
...
select
    cast(infodengue_riodejaneiro_observation_key as varchar) as infodengue_riodejaneiro_observation_key,
