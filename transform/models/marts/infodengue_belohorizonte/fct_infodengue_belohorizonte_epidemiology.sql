{{ config(
    ...
    unique_key='infodengue_belohorizonte_observation_key',
...
with source_rows as (
    ...
    from {{ ref('base_infodengue_belohorizonte') }}
    {% if is_incremental() %}
    where _loaded_at >= (
...
keyed as (
    select
        cast(md5(to_json(struct_pack(
            ...
            record_id := record_id
        ))) as varchar) as infodengue_belohorizonte_observation_key,
        *
    from source_rows
),
deduplicated as (
    ...
    qualify row_number() over (
        partition by infodengue_belohorizonte_observation_key
        order by source_loaded_at desc, source_file desc, source_file_row_number desc, source_row_id desc
    ) = 1
)
select
    cast(infodengue_belohorizonte_observation_key as varchar) as infodengue_belohorizonte_observation_key,
