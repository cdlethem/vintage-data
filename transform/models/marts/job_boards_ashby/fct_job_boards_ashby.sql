...
with source_rows as (
    ...
    from {{ ref('base_job_boards_ashby') }}
),
...
keyed as (
    select
        ...
        ))) as varchar) as job_boards_ashby_key,
        *
    from source_rows
),
...
deduplicated as (
    ...
    qualify row_number() over (
        partition by job_boards_ashby_key
        order by source_loaded_at desc, _source_file desc, _file_row_num desc
    ) = 1
)

select
    cast(job_boards_ashby_key as varchar) as job_boards_ashby_key,
    cast(source as varchar) as source,
