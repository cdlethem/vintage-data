...
with source_rows as (
    select
        ...
        try_cast(salary_min as decimal(18, 2)) as salary_min,
        try_cast(salary_max as decimal(18, 2)) as salary_max,
    ...
    from {{ ref('base_job_boards_recruitee') }}
),
...
keyed as (
    select
        ...
        ))) as varchar) as job_boards_recruitee_key,
        *
    from source_rows
),
...
deduplicated as (
    ...
    qualify row_number() over (
        partition by job_boards_recruitee_key
        order by source_loaded_at desc, _source_file desc, _file_row_num desc
    ) = 1
)

select
    cast(job_boards_recruitee_key as varchar) as job_boards_recruitee_key,
    cast(source as varchar) as source,
    ...
    cast(salary_min as decimal(18, 2)) as salary_min,
    cast(salary_max as decimal(18, 2)) as salary_max,
    cast(salary_currency as varchar) as salary_currency,
    cast(salary_interval as varchar) as salary_interval,
    cast(description as varchar) as description,
    cast(_batch_id as varchar) as _batch_id,
