...
with source_rows as (
    select
        md5(to_json(struct_pack(
            player_user := cast("user" as varchar),
            game_id := cast(id as varchar)
        ))) as chess_archive_key,
        ...
        cast("start" as timestamp with time zone) as started_at,
...
deduplicated as (
    ...
    qualify row_number() over (
        partition by chess_archive_key
