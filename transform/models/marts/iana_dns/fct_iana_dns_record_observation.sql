{{ config(
    materialized='incremental',
    incremental_strategy='merge',
    unique_key='iana_dns_record_observation_key',
    on_schema_change='fail',
    tags=['daily']
) }}

with source_rows as (
    select
        cast(source as varchar) as source,
        cast(id as varchar) as record_id,
        cast(fetched_at as timestamp with time zone) as observed_at,
        cast(owner as varchar) as owner,
        cast(ttl as bigint) as ttl_seconds,
        cast(class as varchar) as record_class,
        cast(type as varchar) as record_type,
        cast(rdata as varchar) as record_data,
        cast(publisher_updated_at as varchar) as publisher_updated_at,
        cast(_row_id as varchar) as source_row_id,
        cast(_batch_id as varchar) as source_batch_id,
        cast(_source_file as varchar) as source_file,
        cast(_file_row_num as bigint) as source_file_row_number,
        cast(_dt as date) as source_date,
        cast(_extract_started_at as timestamp with time zone) as extract_started_at,
        cast(_load_id as varchar) as load_id,
        cast(_loaded_at as timestamp with time zone) as source_loaded_at,
        cast(_content_hash as varchar) as content_hash
    from {{ ref('base_iana_dns') }}
    {% if is_incremental() %}
    where _loaded_at >= (
        select coalesce(max(source_loaded_at), timestamp '1900-01-01')
        from {{ this }}
    )
    {% endif %}
),
ranked as (
    select
        cast(md5(to_json(struct_pack(
            source := source,
            record_id := record_id,
            observed_at := observed_at
        ))) as varchar) as iana_dns_record_observation_key,
        *,
        row_number() over (
            partition by source, record_id, observed_at
            order by source_loaded_at desc, source_file desc,
                     source_file_row_number desc, source_row_id desc
        ) as _dedupe_rank
    from source_rows
)

select
    cast(iana_dns_record_observation_key as varchar) as iana_dns_record_observation_key,
    cast(source as varchar) as source,
    cast(record_id as varchar) as record_id,
    cast(observed_at as timestamp with time zone) as observed_at,
    cast(owner as varchar) as owner,
    cast(ttl_seconds as bigint) as ttl_seconds,
    cast(record_class as varchar) as record_class,
    cast(record_type as varchar) as record_type,
    cast(record_data as varchar) as record_data,
    cast(publisher_updated_at as varchar) as publisher_updated_at,
    cast(source_row_id as varchar) as source_row_id,
    cast(source_batch_id as varchar) as source_batch_id,
    cast(source_file as varchar) as source_file,
    cast(source_file_row_number as bigint) as source_file_row_number,
    cast(source_date as date) as source_date,
    cast(extract_started_at as timestamp with time zone) as extract_started_at,
    cast(load_id as varchar) as load_id,
    cast(source_loaded_at as timestamp with time zone) as source_loaded_at,
    cast(content_hash as varchar) as content_hash
from ranked
where _dedupe_rank = 1
