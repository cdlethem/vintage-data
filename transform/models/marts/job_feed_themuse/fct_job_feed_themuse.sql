...
with source_rows as (
    ...
    from {{ ref('base_job_feed_themuse') }}
),
...
keyed as (
    select
        ...
        ))) as varchar) as job_feed_themuse_key,
        *
    from source_rows
),
...
deduplicated as (
    ...
    qualify row_number() over (
        partition by job_feed_themuse_key
        order by source_loaded_at desc, _source_file desc, _file_row_num desc
    ) = 1
)

select
    cast(job_feed_themuse_key as varchar) as job_feed_themuse_key,
    cast(source as varchar) as source,
