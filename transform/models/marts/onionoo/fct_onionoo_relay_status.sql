{{ config(
    ...
    unique_key='onionoo_relay_status_key',
...
with source_rows as (
    select
        ...
        cast(id as varchar) as relay_id,
        cast(fetched_at as timestamp with time zone) as observed_at,
        cast(nickname as varchar) as nickname,
        cast(addresses as json) as addresses,
        cast(running as boolean) as running,
...
deduplicated as (
    ...
    qualify row_number() over (
        partition by onionoo_relay_status_key
        order by source_loaded_at desc, source_file desc, source_file_row_number desc, source_row_id desc
    ) = 1
)

select
    cast(onionoo_relay_status_key as varchar) as onionoo_relay_status_key,
    cast(source as varchar) as source,
    cast(relay_id as varchar) as relay_id,
